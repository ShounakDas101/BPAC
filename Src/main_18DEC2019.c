...
void  SPI_MasterInit(void); //CPOL=1, CPHA=1
void  SPI_SlaveInit(void); //CPOL=1, CPHA=1
...
I2C_Init();	
	...
	switch(boardName){
		case 0:{
		  CCC0IO_Init();
			SPI_CS1_RESET;//mSec(5000);	
	    SPI_CS2_RESET;//mSec(5000);
	    SPI_CS3_RESET;//mSec(5000);
			//SPI_MasterInit();
			UART2_Init();	
			LCD_Init();
