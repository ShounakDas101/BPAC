...
#define dropTCFXR	  't'
#define liftTCFXR   'T'
#define dropASGNCPR 'a'
#define liftASGNCPR 'A'
#define dropBLR			'b'
#define liftBLR			'B'
...
#define tgtBLLpres 0xC1  //it should be 0xC1
//TGT:Bell Alart Sent ..  UART .. ‘B’
#define tgtBLLcomm 'B'
//TGT:TGT and Bell Pressed   (TGTbell) .. INP_RELAY.. BPNR, TGTXR, ASGNCR, BIPR1, BIPR2 picked-up
#define tgtTGTbell 0xD3  //it should be 0xD3
//TGT:Train Sending Request .. UART ..’T’  
#define tgtTGTcomm 'T'
//TCF:Line Clear .. INP_RELAY .. TCFR, ASGNCPR, HSGNCR, BIPR1, BIPR2 picked-up
#define tcfCLRline 0xA9
...
//TGT:TGT Line occupied ..  UART .. ’t’
#define tgtTONline 'O'
//TCF:TCF Green .. INP_RELAY .. HSGNCR picked-up ASGNCPR high TCFR high
//#define  tcf
//TGT:TGT Green .. UART .. ‘G’
//#define tgt
	...
	switch(boardName){
      ...
      mSec(2000);
          ...
          LCD_MSG[4] = (uint8_t)usart_recv_blocking();
				  ...
				  if(inp_status == tgtBLLpres && com_status==LINK) {
					...
					if(inp_status == tgtTGTbell && com_status==tcfCLRcomm){
						  ...
						  mSec(200);
							...
							LCD_Send_string_Line1("TGT:TGTbell OFF.");
				  ...
				  usart_send_blocking(tgtTONline);
				  usart_send_blocking(tgtTONline);
				  usart_send_blocking(tgtTONline);
				  ...
				  usart_send_blocking(liftASGNCPR);
				  usart_send_blocking(liftASGNCPR);
				  usart_send_blocking(liftASGNCPR);
					LCD_Send_string_Line1("TGT:TOL .full...");
          ...
          // Wait for HSGNCR dropped in Receiving station				
			    ...
			    relayStatus3 = Command(CPU3,TGTZR,SET);
					...
					LCD_Send_string_Line1("Attention SM....");	
					LCD_Send_string_Line2("BLR ON ringing..");
					mode=NOPE;
					}
					if(com_status!='B'){
					...
					if(INP_RELAY==HSGNCR  && com_status=='T'){
      ...
      // Wait for ASGNCR dropped in Sending station				
  			...
  			LCD_MSG[6] = (uint8_t)usart_recv_blocking();
  			LCD_MSG[8] = (uint8_t)usart_recv_blocking();	
 					status=twoOOthree(LCD_MSG[4],LCD_MSG[6],LCD_MSG[8])	;
					if(status=='A'){
					LCD_Send_string_Line1("Sending Station.");	
					LCD_Send_string_Line2("ASGNCR dropped..");
					break;	
					}
				}while(1);
      ...
      // Wait for ASGNCR dropped in Sending station				
  			...
  			LCD_MSG[8] = (uint8_t)usart_recv_blocking();	
					...
					if(status=='A'){
					LCD_Send_string_Line1("Sending Station.");	
					LCD_Send_string_Line2("ASGNCPR pickedup");
					break;	
					}
				}while(1);
	 		relayStatus=twoOOthree(relayStatus1,relayStatus2,relayStatus3);
      relayStatus1 = Command(1,ASGNCR,SET);
			relayStatus2 = Command(2,ASGNCR,SET);
			relayStatus3 = Command(3,ASGNCR,SET);
			relayStatus=twoOOthree(relayStatus1,relayStatus2,relayStatus3);
			mSec(1000);	
			// wait till HSGNCR drop
			do{
					relayStatus1 = Command(CPU1,RELAY,READ);
					relayStatus2 = Command(CPU2,RELAY,READ);
					relayStatus3 = Command(CPU3,RELAY,READ);
					OTP_RELAY = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
					INP_RELAY =	twoOOthree(relayStatus1>>8, relayStatus2>>8, relayStatus3>>8);			
					status = INP_RELAY & HSGNCR;
				  //if HSGNCR drops then inform sending station
				  if(status==0x00){
				  usart_send_blocking('A');
				  usart_send_blocking('A');
				  usart_send_blocking('A');
		   	}
			}while(status!=0x00);	
			LCD_Send_string_Line2("TCF:HSGNCRdroppd");
				
      relayStatus1 = Command(1,TGTXR,SET);
			relayStatus2 = Command(2,TGTXR,SET);
			relayStatus3 = Command(3,TGTXR,SET);
			relayStatus=twoOOthree(relayStatus1,relayStatus2,relayStatus3);
			mSec(2000);	
			relayStatus1 = Command(1,TGTXR,RESET);
			relayStatus2 = Command(2,TGTXR,RESET);
			relayStatus3 = Command(3,TGTXR,RESET);
			relayStatus=twoOOthree(relayStatus1,relayStatus2,relayStatus3);
...
// Check link failure if link OK set LFR	
      ...
      // Now TGTXR picked-up digitalLineClear received
			...
			relayStatus2 = Command(CPU2,TGTZR,SET);
			relayStatus3 = Command(CPU3,TGTZR,SET);
			LCD_Send_string_Line2("TGT NORMposition");	
			...
			do{
				  ...
				  usart_send_blocking(dropTCFXR);
					...
					LCD_Send_string_Line2("TGT:TCFR latched");
			// wait till ASGNCR drops
			do{
					relayStatus1 = Command(CPU1,RELAY,READ);
					relayStatus2 = Command(CPU2,RELAY,READ);
					relayStatus3 = Command(CPU3,RELAY,READ);
					OTP_RELAY = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
					INP_RELAY =	twoOOthree(relayStatus1>>8, relayStatus2>>8, relayStatus3>>8);			
					status = INP_RELAY & ASGNCR;
				  //if ASGNCR drops then ask receiving station to drop ASGNCPR
				  if(status==0x00){
				  usart_send_blocking(dropASGNCPR);
				  usart_send_blocking(dropASGNCPR);
				  usart_send_blocking(dropASGNCPR);
					}
				}while(status!=0x00);	
			  LCD_Send_string_Line2("TGT:ASGNCR dropd");
			
			// wait till ASGNCR picksup by SM
			do{
					relayStatus1 = Command(CPU1,RELAY,READ);
					relayStatus2 = Command(CPU2,RELAY,READ);
					relayStatus3 = Command(CPU3,RELAY,READ);
			...
			LCD_Send_string_Line2("TGT:ASGNCRlifted");
      ...
      // Wait for HSGNCR dropped in Receiving station				
  			...
  			LCD_MSG[8] = (uint8_t)usart_recv_blocking();	
					...
					LCD_Send_string_Line2("Line Clear .....");
					break;	
					}
				}while(1);					
...
void  recvingStation(void){
      ...
      // Wait for ASGNCR dropped in Sending station				
  			...
  			LCD_MSG[8] = (uint8_t)usart_recv_blocking();	
					...
					if(status=='A'){
					LCD_Send_string_Line1("Sending Station.");	
					LCD_Send_string_Line2("ASGNCP pickedup.");
					break;	
					}
				}while(1);
	 		relayStatus=twoOOthree(relayStatus1,relayStatus2,relayStatus3);
