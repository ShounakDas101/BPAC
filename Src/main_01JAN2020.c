...
#define TWO_OO_THREE(A, B, C) ((A && B) || (B && C) || (C && A))
...
const uint8_t	XXXR   =0x80;
const uint8_t SET		 ='S';
const uint8_t RESET  ='R';

uint8_t inputCPU1_CCC0[3];// will be sent by CPUx and used by CCC0
uint8_t inputCPU2_CCC0[3];// will be sent by CPUx and used by CCC0
uint8_t inputCPU3_CCC0[3];// will be sent by CPUx and used by CCC0
uint8_t fedbkCPU1_CCC0[3];// will be sent by CPUx and used by CCC0
uint8_t fedbkCPU2_CCC0[3];// will be sent by CPUx and used by CCC0
uint8_t fedbkCPU3_CCC0[3];// will be sent by CPUx and used by CCC0
...
void Command(uint8_t cpu, uint8_t relay, uint8_t setreset);
void  Respond(uint8_t);
...
I2C_Init();	
	...
	switch(boardName){
      ...
      mSec(5000);
			...
			//LCD_Send_string_Line1("UFSBI-Operating.");
...
void Command(uint8_t cpu, uint8_t relay, uint8_t setreset){
	if(cpu==1){
	    SPI_CS1_RESET;uSec(500);// wait , CPU1 reads all the relay status 
		  ...
		  inputCPU1_CCC0[0]=SPI_SendRecv(setreset);
			inputCPU1_CCC0[1]=SPI_SendRecv(setreset);
			inputCPU1_CCC0[2]=SPI_SendRecv(setreset);
			fedbkCPU1_CCC0[0]=SPI_SendRecv(relay);
			fedbkCPU1_CCC0[1]=SPI_SendRecv(relay);
		  fedbkCPU1_CCC0[2]=SPI_SendRecv(relay);
			SPI1_CR1 &= ~SPI_CR1_SPE;// disable SPI
      SPI_CS1_SET;uSec(500);	
			//LCD_Send_string_Line2("COMMAND CPU1");
	}
	else if(cpu==2){						
			SPI_CS2_RESET;uSec(500);// wait , CPU2 reads all the relay status
			SPI1_CR1 |= SPI_CR1_SPE;// enable SPI	
			inputCPU2_CCC0[0]=SPI_SendRecv(setreset);
			inputCPU2_CCC0[1]=SPI_SendRecv(setreset);
			inputCPU2_CCC0[2]=SPI_SendRecv(setreset);
			fedbkCPU2_CCC0[0]=SPI_SendRecv(relay);
			fedbkCPU2_CCC0[1]=SPI_SendRecv(relay);
		  fedbkCPU2_CCC0[2]=SPI_SendRecv(relay);
   		SPI1_CR1 &= ~SPI_CR1_SPE;// disable SPI	
		  SPI_CS2_SET;uSec(500);
			...
			SPI1_CR1 |= SPI_CR1_SPE;// enable SPI	
			inputCPU3_CCC0[0]=SPI_SendRecv(setreset);
			inputCPU3_CCC0[1]=SPI_SendRecv(setreset);
			inputCPU3_CCC0[2]=SPI_SendRecv(setreset);
			fedbkCPU3_CCC0[0]=SPI_SendRecv(relay);
			fedbkCPU3_CCC0[1]=SPI_SendRecv(relay);
		  fedbkCPU3_CCC0[2]=SPI_SendRecv(relay);
  		SPI1_CR1 &= ~SPI_CR1_SPE;// disable SPI
...
void Respond(uint8_t cpu){
	...
	if (POWER_FRONT_STATUS) 	inputFront |=  (1<<0);
	else                      inputFront &= ~(1<<0);
	if (TGTXR_FRONT_STATUS) 	inputFront |=  (1<<1);
	else											inputFront &= ~(1<<1);
	if (CANCO_FRONT_STATUS)   inputFront |=  (1<<2);
	else											inputFront &= ~(1<<2);
	if (TCFR_FRONT_STATUS) 		inputFront |=  (1<<3);
	else											inputFront &= ~(1<<3);
	if (ASGNCR_FRONT_STATUS)  inputFront |=  (1<<4);
	else											inputFront &= ~(1<<4);
	if (HSGNCR_FRONT_STATUS)  inputFront |=  (1<<5);
	else											inputFront &= ~(1<<5);
	if (BPNR_FRONT_STATUS) 		inputFront |=  (1<<6);
  else											inputFront &= ~(1<<6);
	if (BIPR1_FRONT_STATUS) 	inputFront |=  (1<<7);
	else											inputFront &= ~(1<<7);
	
	if (!POWER_BACK_STATUS) 	inputBack |=  (1<<0);
	else                      inputBack &= ~(1<<0);
	if (!TGTXR_BACK_STATUS) 	inputBack |=  (1<<1);
	else											inputBack &= ~(1<<1);
	if (!CANCO_BACK_STATUS)   inputBack |=  (1<<2);
	else											inputBack &= ~(1<<2);
	if (!TCFR_BACK_STATUS) 		inputBack |=  (1<<3);
	else											inputBack &= ~(1<<3);
	if (!ASGNCR_BACK_STATUS)  inputBack |=  (1<<4);
	else											inputBack &= ~(1<<4);
	if (!HSGNCR_BACK_STATUS)  inputBack |=  (1<<5);
	else											inputBack &= ~(1<<5);
	if (!BPNR_BACK_STATUS) 		inputBack |=  (1<<6);
  else											inputBack &= ~(1<<6);
	if (!BIPR1_BACK_STATUS) 	inputBack |=  (1<<7);
	else											inputBack &= ~(1<<7);
	
	if (LFR_FRONT_STATUS) 	  fedbkFront |=  (1<<0);
	else                      fedbkFront &= ~(1<<0);
	if (TCFXR_FRONT_STATUS) 	fedbkFront |=  (1<<1);
	else											fedbkFront &= ~(1<<1);
	if (TCFCR_FRONT_STATUS)   fedbkFront |=  (1<<2);
	else											fedbkFront &= ~(1<<2);
	if (TGTYR_FRONT_STATUS) 	fedbkFront |=  (1<<3);
	else											fedbkFront &= ~(1<<3);
	if (ASGNCPR_FRONT_STATUS) fedbkFront |=  (1<<4);
	else											fedbkFront &= ~(1<<4);
	if (TGTZR_FRONT_STATUS)   fedbkFront |=  (1<<5);
	else											fedbkFront &= ~(1<<5);
	if (BLR_FRONT_STATUS) 		fedbkFront |=  (1<<6);
  else											fedbkFront &= ~(1<<6);
	if (BIPR2_FRONT_STATUS) 	fedbkFront |=  (1<<7);
	else											fedbkFront &= ~(1<<7);
	
	if (!LFR_BACK_STATUS) 	  fedbkBack |=  (1<<0);
	else                      fedbkBack &= ~(1<<0);
	if (!TCFXR_BACK_STATUS) 	fedbkBack |=  (1<<1);
	else											fedbkBack &= ~(1<<1);
	if (!TCFCR_BACK_STATUS)   fedbkBack |=  (1<<2);
	else											fedbkBack &= ~(1<<2);
	if (!TGTYR_BACK_STATUS) 	fedbkBack |=  (1<<3);
	else											fedbkBack &= ~(1<<3);
	if (!ASGNCPR_BACK_STATUS) fedbkBack |=  (1<<4);
	else											fedbkBack &= ~(1<<4);
	if (!TGTZR_BACK_STATUS)   fedbkBack |=  (1<<5);
	else											fedbkBack &= ~(1<<5);
	if (!BLR_BACK_STATUS) 		fedbkBack |=  (1<<6);
  else											fedbkBack &= ~(1<<6);
	if (!BIPR2_BACK_STATUS) 	fedbkBack |=  (1<<7);
	else											fedbkBack &= ~(1<<7);
	
	
	//if(inputFront != (~inputBack))
	//ErrorHandle(INP1_fail);
	//if(fedbkFront != (~fedbkBack))
	//ErrorHandle(INP1_fail);
	
	
//				if(cpu==1){
				SPI1_CR1 |= SPI_CR1_SPE;// enable SPI
			  sr1 =SPI_SendRecv(inputFront);LCD_MSG[0]=sr1;
				sr2 =SPI_SendRecv(inputFront);LCD_MSG[1]=sr2;
				sr3 =SPI_SendRecv(inputFront);LCD_MSG[2]=sr3;
				out1=SPI_SendRecv(fedbkFront);LCD_MSG[3]=0x30+out1;
				out2=SPI_SendRecv(fedbkFront);LCD_MSG[4]=0x30+out2;
				out3=SPI_SendRecv(fedbkFront);LCD_MSG[5]=0x30+out3;
				SPI1_CR1 &= ~SPI_CR1_SPE;// disable SPI
				out= out1;//TWO_OO_THREE(out1,out2,out3);LCD_MSG[9]=0x30+out;
				setreset=sr1;//TWO_OO_THREE(sr1,sr2,sr3);LCD_MSG[10]=0x30+setreset;	
...
//uint8_t Set(uint8_t relay){
//  uint8_t temp;
//	temp = msgCCC0_CPU | relay;
//	return temp;
//}
//uint8_t Reset(uint8_t relay){
//  uint8_t temp;
//	temp = msgCCC0_CPU & (~relay);
//	return temp;
//}
