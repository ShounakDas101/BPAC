...
#define SENDING    0x90   //input=10010010
#define TIMEOUT    0x03
#define OK				 0x00	


// Various states UFSBI follows
//TGT:Normal Relay Condition .. INP_RELAY.. BIPR1, BIPR2, ASGNCR picked-up
#define tgtINPnorm 0x91    
//TGT:Normal Relay Condition .. OTP_RELAY.. LFR,   TGTZR, POWER  picked-up
#define tgtOTPnorm 0xA1
//TGT:Bell Pressed (BLLpres) .. INP_RELAY.. BPNR, BIPR1, BIPR2 picked-up
#define tgtBLLpres 0xC1
//TGT:TGT and Bell Pressed   .. INP_RELAY.. BPNR, TGTXR, ASGNCR, BIPR1, BIPR2 picked-up
#define tgtTGTbell 0xD3
//TGT:TGT Axel counter dropped .. INP_RELAY.. BIPR1, BIPR2 picked-up, ASGNCR dropped
#define tgtAXLdrop 0x81
//TGT:TGT Cancel Co-operation INP_RELAY.. BIPR1, BIPR2, CANCO, ASGNCR pick-up
#define tgtCANcoop 0x95
//TCF:Normal Relay Condition .. INP_RELAY.. BIPR1, BIPR2, HSGNCR picked-up
#define tcfINPnorm 0xA1 
//TCF:Normal Relay Condition .. OTP_RELAY.. LFR, ASGNCPR, POWER  picked-up
#define tcfOTPnorm 0x91 
//TCF:Line Clear .. INP_RELAY .. TCFR, ASGNCPR, HSGNCR, BIPR1, BIPR2 picked-up
//#define tcfCLRline 0xXX
//TCF:Line Clear .. INP_RELAY .. BIPR1, BIPR2 picked-up HSGNCR drop
#define tcfTRNarrv  0x81
//TGT:TGT Cancel Co-operation INP_RELAY.. BIPR1, BIPR2, CANCO, ASGNCR pick-up
#define tcfCANbell 0x95


// Message passing Signals between Stations
//LINKOK..  UART .. 'L'
#define tgtLNKcomm 'L'
//TGT:Bell Alart Sent ..  UART .. ‘B’
#define tgtBLLcomm 'B'
//TGT:Train Sending Request .. UART ..’T’  
#define tgtTGTcomm 'T'
//TCF:Line Clear Granted .. UART .. ‘C’
#define tcfCLRcomm 'C'
//TGT:TGT Line occupied ..  UART .. 'O'
#define tgtTOLcomm 'O'
//TGT:TGT Line occupied ASGNCR dropped after TOL..  UART .. 'O'
#define tgtOCUcomm 'o'
//TCF: Train arrived .. UART    ‘A’  TCFR dropped/delatched
#define tcfARRcomm 'A'
//TGT:TGT CANCEL COOP  UART .. 'c'
#define tgtCANcomm 'c'
//TCF:TCF CANCEL +BELL UART .. 'c'
#define tcfCANcomm 'b'
//TCF:Bell Alart Sent ..  UART .. ‘B’
#define tcfBLLcomm 'B'
//TCF: Line closed 120s delay ‘F’
#define tcfLINclos 'F' 
//TGT:TGT Axel counter high   INP_RELAY ASGNCR  pick-up

//TGT: Line closed BIPR1, BIPR2, ASGNCR, TGTZR   picked-up
//TCF: Line closed BIPR1, BIPR2, HSGNCP, ASGNCPR picked-up
...
const uint8_t CCC0=0x00;
const uint8_t	CPU1=0x01;
const uint8_t	CPU2=0x02;
const uint8_t	CPU3=0x03;
...
const uint8_t BELL   =0x40;
const uint8_t TGTC   =0xD2;
const uint8_t CLEAR  ='C';
const uint8_t NOPE	 =0xFF;
const uint8_t LINK   ='L';
...
volatile uint8_t inp_status; // INP_RELAY status
volatile uint8_t otp_status; // OTP_RELAY status
volatile uint8_t com_status; // communication status (Station Master command for TGT/TCF)
uint8_t mode;
uint8_t rx, tx;
uint32_t i;
uint32_t time;
...
uint8_t BlockNormal(void);
uint8_t LinkOK(void);
uint8_t PowerOK(void);
uint8_t BlockNormal(void);
uint8_t BellAlert(void);
	...
	switch(boardName){
      ...
      mSec(2000);
		  ...
		  if(status==OK)
			...
			LCD_Send_string_Line2("B24powerlineOFF");	
			// Set UFSBI in Normal Condition
			status=BlockNormal();
			if(status==OK)LCD_Send_string_Line1("BI NORMcondition");
			
			//while(1){
	  	// Check link failure if link OK set LFR
			status=LinkOK();
			if(status==OK)	         LCD_Send_string_Line2("LINK......... OK");
			else if (status==TIMEOUT)LCD_Send_string_Line2("LINK........FAIL");
      else										 LCD_Send_string_Line2("LINK...doubtfull");	
          ...
          LCD_MSG[4] = (uint8_t)usart_recv_blocking();
					LCD_MSG[6] = (uint8_t)usart_recv_blocking();
					LCD_MSG[8] = (uint8_t)usart_recv_blocking();
					com_status=twoOOthree(LCD_MSG[4],LCD_MSG[6],LCD_MSG[8]);
				  /////////////////////////////////////////////////////////////////
				  // Normal Condition
				  inp_status=INP_RELAY & BIOK;
				  otp_status=OTP_RELAY & 0x81;
				  if(inp_status == BIOK && com_status==LINK)
				  {
					LCD_Send_string_Line1("TGT: IDLE MODE..");	
					LCD_Send_string_Line2("TCF: IDLE MODE..");
					}
				  //////////////////////////////////////////////////////////////////
				  inp_status= INP_RELAY & tgtBLLpres; // this should not be and-ed
				  if(inp_status == tgtBLLpres && com_status==LINK) {
						BellAlert();
					  LCD_Send_string_Line1("TGT:Only bell...");	
					  mode=NOPE;
					}
				  //////////////////////////////////////////////////////////////////
					inp_status=INP_RELAY & tgtTGTbell;
					if(inp_status == tgtTGTbell && com_status==LINK){
							usart_send_blocking(tgtTGTcomm);
							usart_send_blocking(tgtTGTcomm);
							usart_send_blocking(tgtTGTcomm);
						  LCD_Send_string_Line1("TGT:TGT+BELL....");	
					}
				  /////////////////////////////////////////////////////////////////
					inp_status=INP_RELAY & tgtTGTbell;
					if(inp_status == tgtTGTbell && com_status==tcfCLRcomm){
							// Now TGTXR picked-up digitalLineClear received
						  ...
						  mSec(200);
							...
							// wait till TGTXR drops (if TGT and BELL Released)
							do{
							relayStatus1 = Command(CPU1,RELAY,READ);
							relayStatus2 = Command(CPU2,RELAY,READ);
							relayStatus3 = Command(CPU3,RELAY,READ);
							OTP_RELAY = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
							INP_RELAY =	twoOOthree(relayStatus1>>8, relayStatus2>>8, relayStatus3>>8);			
							inp_status = INP_RELAY & TGTXR;
//							//if TGTXR drops then ask receiving station to drop TCFXR
//							if(inp_status==0x00){
//							usart_send_blocking(dropTCFXR);
//							usart_send_blocking(dropTCFXR);
//							usart_send_blocking(dropTCFXR);
//							}
							}while(inp_status!=0x00);
							LCD_Send_string_Line1("TGT:TGT LINEfree");
							
						}
					//////////////////////////////////////////////////////////////////
					// wait till ASGNCR drops in Line clear condition
					inp_status = INP_RELAY & ASGNCR;
				  //if ASGNCR drops then ask receiving station to drop ASGNCPR
				  if(inp_status == 0x00 && com_status == tcfCLRcomm){
				  usart_send_blocking(tgtTOLcomm);
				  usart_send_blocking(tgtTOLcomm);
				  usart_send_blocking(tgtTOLcomm);
				  LCD_Send_string_Line1("TGT:TRAINonLINE.");
					}
					/////////////////////////////////////////////////////////////////
					// wait till ASGNCR picksup by SM in Line clear condition
					inp_status = INP_RELAY & ASGNCR;
				  //if ASGNCR pick-up then ask receiving station to lift ASGNCPR
				  if(inp_status==ASGNCR  && com_status == tcfCLRcomm){
				  usart_send_blocking(tgtOCUcomm);
				  usart_send_blocking(tgtOCUcomm);
				  usart_send_blocking(tgtOCUcomm);
					LCD_Send_string_Line1("TGT:Train in sec");
					// Sending station will drop TGTYR and BTSR drops automatically
					relayStatus1 = Command(CPU1,TGTYR,RESET);
					relayStatus2 = Command(CPU2,TGTYR,RESET);
					relayStatus3 = Command(CPU3,TGTYR,RESET);		
					//mSec(1000);						
					...
					////////////////////////////////////////////////////////////////
					if(com_status==tcfARRcomm){	
          // Wait for HSGNCR dropped in Receiving station				
				  relayStatus1 = Command(CPU1,TGTZR,SET);
			    relayStatus2 = Command(CPU2,TGTZR,SET);
			    relayStatus3 = Command(CPU3,TGTZR,SET);
					LCD_Send_string_Line1("Attention SM....");	
					LCD_Send_string_Line2("TGT Normal condn");	
      		}
					//////////////////////////////////////////////////////////////
					// Cancel Co-operation button pressed, ASGNCR pick-up condition
					inp_status = INP_RELAY & tgtCANcoop;
				  //if ASGNCR high with CANCO button pressed
				  if(inp_status==tgtCANcoop){
								//send cancel co-op code to receiving station	
								usart_send_blocking(tgtCANcomm);
								usart_send_blocking(tgtCANcomm);
								usart_send_blocking(tgtCANcomm);
								LCD_Send_string_Line1("TGT:CANCEL-CO-OP");
								// receiving station TCFCR picks-up 
								// receiving statin press BELL+CANCEL   picks-up BPNR	and sends cancel code
								// sending station wait till receive cancel code from receiving station
								do{	
								LCD_MSG[4] = (uint8_t)usart_recv_blocking();
								LCD_MSG[6] = (uint8_t)usart_recv_blocking();
								LCD_MSG[8] = (uint8_t)usart_recv_blocking();
								com_status=twoOOthree(LCD_MSG[4],LCD_MSG[6],LCD_MSG[8]);		
								if(com_status=='b'){			//tcfCANcomm			
										// Sending station will pick-up local BPNR after receiving cancel code
										relayStatus1 = Command(CPU1,BLR,SET);
										relayStatus2 = Command(CPU2,BLR,SET);
										relayStatus3 = Command(CPU3,BLR,SET);
										break;
										}						
								}while(1);
								// still the sending station is holding can-co button
								// receiving station releases CANCEL+BELL button sends bell code
								// sending station wait to receive bell code
								do{	
								LCD_MSG[4] = (uint8_t)usart_recv_blocking();
								LCD_MSG[6] = (uint8_t)usart_recv_blocking();
								LCD_MSG[8] = (uint8_t)usart_recv_blocking();
								...
								if(com_status=='L'){						
										// Sending station will pick-up local BPNR after receiving cancel code
										relayStatus1 = Command(CPU1,BLR,RESET);
										relayStatus2 = Command(CPU2,BLR,RESET);
										relayStatus3 = Command(CPU3,BLR,RESET);
										break;
										}						
								}while(1);
								// wait for code from sending station, so that TCFR has been dropped 
								do{	
								LCD_MSG[4] = (uint8_t)usart_recv_blocking();
								LCD_MSG[6] = (uint8_t)usart_recv_blocking();
								LCD_MSG[8] = (uint8_t)usart_recv_blocking();
...
/*********************train coming from*******************************/					
					...
					relayStatus3 = Command(CPU3,BLR,SET);
					//LCD_Send_string_Line1("Attention SM....");	
					LCD_Send_string_Line2("TCF:Bell ringing");
					}
					if(com_status!='B'){
					//LCD_Send_string_Line1("UFSBI IDLE MODE.");	
					//LCD_Send_string_Line2("Waiting for SM..");
					relayStatus1 = Command(CPU1,BLR,RESET);
					relayStatus2 = Command(CPU2,BLR,RESET);
					relayStatus3 = Command(CPU3,BLR,RESET);	
					}
					//////////////////////////////////////////////////////////////
					inp_status=INP_RELAY & tcfINPnorm;
					if(inp_status==tcfINPnorm  && com_status=='T'){
					...
					mSec(1000);
					LCD_Send_string_Line2("TCF:TCFR latched");	
					// TCFR Latches
			
					// Send LINE CLEAR GRANTED CODE to sending station
					usart_send_blocking(tcfCLRcomm);
					usart_send_blocking(tcfCLRcomm);	
					usart_send_blocking(tcfCLRcomm);	
					}
					////////////////////////////////////////////////////////////////
          ...
          // Wait for ASGNCR dropped in Sending station				
					if(com_status==tgtTOLcomm){
					relayStatus1 = Command(1,ASGNCPR,RESET);
					relayStatus2 = Command(2,ASGNCPR,RESET);
					relayStatus3 = Command(3,ASGNCPR,RESET);
					LCD_Send_string_Line2("TCF:LINEoccupied");
					}
					
					// Wait for ASGNCR dropped in Sending station				
					if(com_status==tgtOCUcomm){
					relayStatus1 = Command(1,ASGNCPR,SET);
					relayStatus2 = Command(2,ASGNCPR,SET);
					relayStatus3 = Command(3,ASGNCPR,SET);
					LCD_Send_string_Line2("TCF:ASGNCPRdrop.");
					}
					// train enters home, HSGNCR drops
					inp_status=INP_RELAY & HSGNCR;
					if(inp_status==0x00){
					usart_send_blocking(tcfARRcomm);
					usart_send_blocking(tcfARRcomm);	
					usart_send_blocking(tcfARRcomm);
					relayStatus1 = Command(1,TCFXR,SET);
					relayStatus2 = Command(2,TCFXR,SET);
				  ...
				  mSec(1000);
					...
					relayStatus1 = Command(CPU1,TCFCR,SET);
					relayStatus2 = Command(CPU2,TCFCR,SET);
					relayStatus3 = Command(CPU3,TCFCR,SET);
					mSec(2000);
					relayStatus1 = Command(CPU1,ASGNCPR,SET);
					relayStatus2 = Command(CPU2,ASGNCPR,SET);
					relayStatus3 = Command(CPU3,ASGNCPR,SET);	
					//LCD_Send_string_Line1("Attention SM....");	
					LCD_Send_string_Line2("TCF:CANCOrequest");
					//wait for CANCEL and BELL button press in Receiving station
				  do{
					relayStatus1 = Command(CPU1,RELAY,READ);
					relayStatus2 = Command(CPU2,RELAY,READ);
					relayStatus3 = Command(CPU3,RELAY,READ);
					OTP_RELAY = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
					INP_RELAY =	twoOOthree(relayStatus1>>8, relayStatus2>>8, relayStatus3>>8);
          inp_status=INP_RELAY & BELL;
					if(inp_status==BELL){	
								usart_send_blocking(tcfBLLcomm);
								usart_send_blocking(tcfBLLcomm);	
								usart_send_blocking(tcfBLLcomm);
								
//								relayStatus1 = Command(CPU1,TCFXR,RESET);
//								relayStatus2 = Command(CPU2,TCFXR,RESET);
//								relayStatus3 = Command(CPU3,TCFXR,RESET);
								break;
								}						
					}while(1);
					
					// wait till SM releases CANCEL and BELL button in receiving station
					// BLR drops in sending station bell sound mutes
					 do{
					relayStatus1 = Command(CPU1,RELAY,READ);
					relayStatus2 = Command(CPU2,RELAY,READ);
					relayStatus3 = Command(CPU3,RELAY,READ);
          ...
          inp_status=INP_RELAY & BELL;
					...
					// once TCFR drops in receiving station 
					// send tcfLINclos
					if(com_status==tcfLINclos){
						relayStatus1 = Command(CPU1,TGTZR,SET);
						relayStatus2 = Command(CPU2,TGTZR,SET);
						relayStatus3 = Command(CPU3,TGTZR,SET);	
					}
					...
					// once TCFR drops in receiving station 
					// send tcfLINclos
					inp_status=INP_RELAY & TCFR;
					if(inp_status==0x00){
						relayStatus1 = Command(CPU1,TCFCR,RESET);
						relayStatus2 = Command(CPU2,TCFCR,RESET);
						relayStatus3 = Command(CPU3,TCFCR,RESET);	
					}
...
uint8_t IdentifyBoard(void){
	...
	// Check for X0000000 00X00000 of GPIOH_IDR
	     if((GPIOA_IDR & 0x1800) == 0x0000) return 0x00;//boardName=CCC0;
	else if((GPIOA_IDR & 0x1800) == 0x1000) return 0x01;//boardName=CPU1;
	else if((GPIOA_IDR & 0x1800) == 0x0800) return 0x02;//boardName=CPU2;
	else if((GPIOA_IDR & 0x1800) == 0x1800) return 0x03;//boardName=CPU3;
	else 																		return 0x04;//boardName=XXX0;
...
uint8_t LinkOK(void){
	time=0;	
do{
			LCD_Send_string_Line1("Wait LINK????...");
			for(i=0;i<16;i++){		
    	usart_send_blocking('L');
			mSec(200);	
			rx=(uint8_t)usart_recv_blocking();	
			LCD_MSG[i]=rx;	
			mSec(200);
			}
			LCD_Send_string_Line1(LCD_MSG);
			
	    if(LCD_MSG[5]=='L')break;
			time++;
			}while(time<4);
			LCD_Send_string_Line2("LINK...doubt....");	
			//mSec(2000);	
      if(time==TIMEOUT){
			relayStatus1 = Command(CPU1,LFR,RESET);
			relayStatus2 = Command(CPU2,LFR,RESET);
			relayStatus3 = Command(CPU3,LFR,RESET);
			return TIMEOUT;
			}
...
uint8_t PowerOK(void){
	        ...
	        LCD_Send_string_Line2("press BI-OK  ...");
					}while(temp != BIOK);	
return OK;	
...
uint8_t BellAlert(void)
{
	  	usart_send_blocking('B');
			mSec(200);	
			rx=OK;//usart_recv_blocking();	
	    return rx; 
}
