...
I2C_Init();	
	...
	switch(boardName){
      ...
      mSec(2000);
			...
			//LCD_Send_string_Line2(CPU1_ready);// "UFSBI-Operating.");	
			mSec(5000);	
			...
			//mSec(3000);
			//while(1) usart_send_blocking(0x5C);
		
			while(1){
			relayStatus1 = Command(CPU1,RELAY,READ);
			relayStatus2 = Command(CPU2,RELAY,READ);
			relayStatus3 = Command(CPU3,RELAY,READ);
			relayStatus_all = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
      //displayGPIO(relayStatus_all,relayStatus_all>>8,relayStatus_all>>16,relayStatus_all>>24);
		  displayGPIO(relayStatus1,relayStatus2,relayStatus3,relayStatus1>>8);		
			//while((uint8_t)relayStatus_all & (ASGNCR));// | BPNR | TGTXR) );
			//INP_RELAY = twoOOthree((uint8_t)(relayStatus1>>8), (uint8_t)(relayStatus2>>8), (uint8_t)(relayStatus3>>8));
      ...
      relayStatus_all = twoOOthree((uint8_t)relayStatus1, (uint8_t)relayStatus2, (uint8_t)relayStatus3);
      //displayGPIO(relayStatus1,relayStatus2>>8,relayStatus3>>16,relayStatus1>>24);
			//displayGPIO(relayStatus_all,relayStatus_all>>8,relayStatus_all>>16,relayStatus_all>>24);
			displayGPIO(relayStatus1,relayStatus2,relayStatus3,relayStatus1>>8);
