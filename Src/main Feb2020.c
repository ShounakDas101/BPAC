...
I2C_Init();	
	...
	switch(boardName){
	    ...
	    SPI_CS3_SET;//mSec(5000);
			...
			LCD_Send_string_Line2("AKG Electrinics.");	
			
			mSec(2000);
			SPI1_CR1 &= ~SPI_CR1_SPE;// disable SPI
			
			LCD_Send_string_Line1("UFSBI-Operating.");
			LCD_Send_string_Line1("Initializing....");
			mSec(2000);
			
			// switch 24V power supply ON			
		  ...
		  if(status==OK)
			...
			status=BlockNormal();
			if(status==OK)
			LCD_Send_string_Line2("BLOCK NORMAL   ");
			
			// Check link failure if link OK set LFR
			status=LinkOK();		
			if(status==OK)
			LCD_Send_string_Line2("Comm link     OK");
			else
			LCD_Send_string_Line2("Comm link   FAIL");
...
do{
    	...
    	usart_send_blocking('?');
			mSec(200);	
			rx=usart_recv_blocking();	
			mSec(200);
			LCD_MSG[i]=(uint8_t)rx;
			LCD_Send_string_Line2("Wait LINK????");	
